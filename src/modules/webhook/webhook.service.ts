...
export class WebhookService implements OnApplicationBootstrap {
  ...
  async getActiveWebhooks(connectorServers: string[]) {
    if (connectorServers.length === 0) return [];
    const placeholders = connectorServers.map(() => "?").join(", ");
    const sql = `
      ...
      WHERE g.webhook_active > 0 AND g.status = 'active' AND g.api_status= 'enable' 
        AND g.connector_server IN (${placeholders})
    `;
    return this.query(sql, connectorServers);
  }
  ...
  async logPending(params: any, url: string) {
    try {
      ...
      const dataToInsert = {
        ...
        connected_server: params.value?.connector_server || null,
        call_type: params.value?.calltype || null,
        created_at: new Date(),
        sent_at: new Date(),
      };
  ...
  async getWebhookInfo() {
    ...
    this.store.arrWebhookByConnector = {};
    try {
      const connectorServers = this.getConfiguredConnectorServers();
      if (connectorServers.length === 0) {
        this.logger.warn(
          "[Webhook] No AMI_ALIAS_<nn> configured — no webhooks loaded.",
        );
        return;
      }
      const rows = await this.getActiveWebhooks(connectorServers);
      this.logger.log(
        `[SQL Result] Webhook Rows: ${JSON.stringify(rows, null, 2)}`,
      );
      rows.forEach((item: any) => {
        const keyHookName = "webhook-" + item.id;
        const connectorServer = item.connector_server;
        if (!connectorServer || !item.hl_exts_queues) return;
        this.store.arrWebhookByConnector[connectorServer] =
          this.store.arrWebhookByConnector[connectorServer] || {};
        if (this.store.arrWebhookByConnector[connectorServer][keyHookName])
          return;
        const queues: string[] = [],
          extensions: string[] = [],
          did: string[] = [];
        ...
        hlExtsQueues.forEach((hlInfo: any) => {
          ...
          extTmp.forEach((ext: string) => {
            if (extensions.indexOf(ext) === -1 && ext.length < 6)
              extensions.push(ext);
            else if (did.indexOf(ext) === -1 && ext.length > 7) did.push(ext);
            if (item.config != null)
              this.store.arrToken = this.store.arrToken || {};
          });
        ...
        item.queues = queues;
        item.webhook_info =
          item.webhook_info && item.webhook_info !== "null"
            ? JSON.parse(item.webhook_info)
            : {};
        item.webhook_external =
          item.webhook_external && item.webhook_external !== "null"
            ? JSON.parse(item.webhook_external)
            : {};
        item.webhook_url = item.webhook_url ? JSON.parse(item.webhook_url) : {};
        item.webhook_type =
          item.webhook_type && item.webhook_type !== "null"
            ? JSON.parse(item.webhook_type)
            : {};
        item.connector_server = connectorServer;
        this.store.arrWebhook[keyHookName] = item;
        this.store.arrWebhookByConnector[connectorServer][keyHookName] = item;
      });
      if (getRuntimeConfig().logEnabled) {
        this.logger.log(
          `[Webhook] Loaded ${Object.keys(this.store.arrWebhook).length} webhooks for ${connectorServers.length} connector server(s)`,
        );
      }
    } catch (err: any) {
  ...
  getWebhookMapForPbx(pbxId: string): Record<string, any> {
    const connectorServer = this.amiManager.getConnectorServer(pbxId);
    if (!connectorServer) {
      this.logger.warn(
        `[Webhook] PBX ${pbxId}: connector alias not configured — webhook routing skipped.`,
      );
      return {};
    }
    const scoped = this.store.arrWebhookByConnector[connectorServer];
    if (!scoped) {
      this.logger.warn(
        `[Webhook] PBX ${pbxId}: no webhooks loaded for connector_server="${connectorServer}".`,
      );
      return {};
    }
    return scoped;
  ...
  checkPostRequest(params: any): boolean {
    let flag = false;
    const webhook_select = this.findWebhookForParams(params);
    ...
    if (webhook_select["webhook_type"]["callcenter"] === "default") {
      if (webhook_select["webhook_info"].hasOwnProperty(params.object)) {
        let type = "";
        if (
          ["ringing", "hangup", "answered", "completed", "misscall"].indexOf(
            params.event,
          ) > -1
        ) {
          if (params.value.calltype === "InboundExtension")
            type = params.event + "_in";
          else if (params.value.calltype === "OutboundExtension")
            type = params.event + "_out";
        } else if (params.event === "AgentStatus") type = "extstatus";
        if (webhook_select["webhook_info"][params.object].indexOf(type) > -1)
          flag = true;
  ...
  sendPostRequestv2(url: string = "", params: any) {
    const webhook_select = this.findWebhookForParams(params);
    if (!webhook_select || !this.checkPostRequest(params)) return;

    params.value.connector_server =
      webhook_select.connector_server || params.value.connector_server || "";

    if (params.event !== "AgentStatus") {
      if (
        (params.value.fromnumber &&
          params.value.tonumber &&
          params.value.fromnumber.length >= 5) ||
        params.value.tonumber.length >= 5
      ) {
        if (
          typeof params.value.groupid !== "undefined" &&
          params.value.groupid === "710"
        ) {
        ...
        if (getRuntimeConfig().logEnabled)
          this.logger.log(
            `--- ${params.event} : ${getTimeFormat()} - > ${params.value.fromnumber} ${params.value.calltype} ${params.value.tonumber}`,
          );
      ...
      request(this._buildOpts(url, params), (err: any, res: any, body: any) => {
        if (!err && getRuntimeConfig().logEnabled)
          this.logger.log(
            `[SendPostV2 AgentStatus] Response: ${JSON.stringify(body)}`,
          );
      });
      if (BLOCKED_URLS.indexOf(url) > -1) {
  ...
  async sendWebhook(url: string, params: any) {
    ...
    for (let attempt = 1; attempt <= runtimeConfig.maxRetry; attempt++) {
      try {
        ...
        if (runtimeConfig.logEnabled)
          this.logger.log(
            `[Webhook] send success (attempt ${attempt}): ${url}`,
          );
        const responseData =
        ...
        if (runtimeConfig.logEnabled)
          this.logger.log(
            `[Webhook] Attempt ${attempt}/${runtimeConfig.maxRetry} failed: ${url}`,
          );
  ...
  private _logHandler(url: string) {
    return (err: any, res: any, body: any) => {
      ...
      else if (getRuntimeConfig().logEnabled)
        this.logger.log(`RESPONSE POST ${url} ${JSON.stringify(body)}`);
    };
  }
    ...
    if (method === "GET") {
      ...
      request.get(url, (err: any, res: any, body: any) => {
        if (err) this.logger.error(`Error ${err}`);
          ...
          if (getRuntimeConfig().logEnabled)
            this.logger.log(
              `[SendGetRequest] Response: ${JSON.stringify(body)}`,
            );
        }
      });
    }
  ...
  pushCallLog(webhook_select: any, uniqueid: string) {
    ...
    const data = this.store.arrCompleteCall[uniqueid],
      calltype = "calllog";
    const url =
      webhook_select["webhook_external"]["callcenter"][calltype]["link"];
    const config = webhook_select["webhook_external"]["callcenter"]["config"];
    const method =
      webhook_select["webhook_external"]["callcenter"][calltype]["method"];
    ...
    Object.keys(config).forEach((e) => {
      if (config[e]["in"].indexOf(calltype) > -1) {
        switch (e) {
          ...
          case "status":
            option.push(
              config[e].alias + "=" + data.disposition.replace(" ", "_"),
            );
            break;
          case "calldate":
            option.push(
              config[e].alias + "=" + data.starttime.trim().replace(" ", "-"),
            );
            break;
          case "extension":
            option.push(
              config[e].alias +
                "=" +
                (this.store.arrDialState[uniqueid] || {}).extension,
            );
            break;
          case "phone": {
            const stateInfo = this.store.arrDialState[uniqueid] || ({} as any);
            const phoneVal =
              stateInfo.calltype === "Inbound"
                ? stateInfo.fromnumber
                : stateInfo.tonumber;
          ...
          case "recordingfile": {
            let recFile = "";
            if (
              data.disposition.toLowerCase() === "answered" &&
              this.store.arrRecordingFile[uniqueid]
            ) {
              recFile =
                this.store.arrRecordingFile[uniqueid].recordingfile.substr(1);
              const recording_url =
                (this.store.arrDialState[uniqueid] || {}).recordingurl +
                "cvf.php?f=";
  ...
  private getConfiguredConnectorServers(): string[] {
    return this.amiManager.getConnectorServers();
  }

  private findWebhookForParams(params: any): any {
    const connectorServer = params.value?.connector_server || "";
    const webhookMap = connectorServer
      ? this.store.arrWebhookByConnector[connectorServer]
      : this.store.arrWebhook;
    const extension = params.value?.extension;
    if (!extension) return null;
    for (const e in webhookMap) {
      if (webhookMap[e]["extensions"].indexOf(extension) > -1) {
        return webhookMap[e];
      }
    }
    return null;
  ...
  async retryWebhook(id: number) {
    ...
    if (!log) {
      return {
        success: false,
        error: { code: "LOG_NOT_FOUND", message: "Không tìm thấy webhook log" },
      };
    }
    if (log.status !== "failed") {
      return {
        ...
        error: {
          code: "LOG_NOT_FAILED",
          message: "Chỉ retry được log có status failed",
        },
      };
    }
    let params;
    try {
      ...
      return {
        ...
        error: {
          ...
          message: "Payload trong DB không hợp lệ",
        },
      };
    }
    ...
    try {
      ...
      if (err.code === "ECONNREFUSED") {
        if (getRuntimeConfig().logEnabled)
          this.logger.log(
            `[Webhook] Retry failed id=${id}: ${url} - ${err.code}`,
          );
      ...
      this.logger.error(
        `[Webhook] Retry failed id=${id}: ${url} - ${errorMessage}`,
      );
