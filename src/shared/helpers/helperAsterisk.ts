...
export function backupStateAsync(
  store: StoreService,
  backFilePath: string,
  logger: Logger,
) {
  try {
    ...
    const payload = JSON.stringify(
      {
        arrDialState: store.arrDialState,
        arrBranchState: store.arrBranchState,
      },
      null,
      2,
    );
...
export function classifyCall(
  ...
  if (OUTBOUND_CONTEXTS.includes(data.context)) {
    calltype = "Outbound";
  } else if (INTERNAL_CONTEXTS.includes(data.context)) {
    calltype = "Internal";
  } else if (INBOUND_CONTEXTS.includes(data.context)) {
    calltype = "Inbound";
  } else {
    if (calleridnum.length > 5) calltype = "Inbound";
    else if ((data.destcalleridnum || data.exten || "").length > 5)
      calltype = "Outbound";
...
export function isValidChannelForWebhook(
  channel: string,
  destchannel: string,
): boolean {
  const isChannelValid =
    channel.includes("SIP/") ||
    channel.includes("LOCAL") ||
    channel.includes("Local") ||
    channel.includes("PJSIP");
  const isDestChannelValid =
    destchannel.includes("SIP/") ||
    destchannel.includes("LOCAL") ||
    destchannel.includes("Local") ||
    destchannel.includes("PJSIP");
  return isChannelValid && isDestChannelValid;
}

export function buildMasterState(
  data: any,
  calleridnum: string,
  calltype: string,
  channel: string,
  ...
  return {
    ...
    // xử lí calltype = outbound outbound thì did = calleridnum và inbound thì did = destcalleridnum
    did:
      calltype === "Outbound"
        ? data.calleridnum
    ...
    connector_server: webhook_select?.connector_server || "",
...
export function buildBranchState(
  ...
  return {
    ...
    destchannel: destchannel,
    extension:
      master.calltype === "Outbound"
        ? parseChannel(master.channel)
        : destchannel.toLowerCase().startsWith("local/")
          ? " "
          : tonumber_val,
    tonumber: tonumber_val,
    did: master.did,
    destination:
      master.calltype !== "Outbound" ? master.destination || " " : " ",
    starttime: timeFormat,
    status: "initiating",
    webhookurl: master.webhookurl,
    recordingurl: master.recordingurl,
    groupid: master.groupid,
    connector_server: master.connector_server || "",
    callrefid: linkedid,
    linkedid,
...
export function cleanupCallState(
  store: StoreService,
  pbxId: string,
  linkedid: string,
  backFilePath: string,
  ...
  for (const key of Object.keys(store.arrCompleteCall)) {
    if (key.startsWith(`${namespacedLinkedid}::`))
      delete store.arrCompleteCall[key];
  }
  for (const key of Object.keys(store.arrBranchState)) {
    if (key.startsWith(`${namespacedLinkedid}::`))
      delete store.arrBranchState[key];
  }
...
export function extractRecordingInfo(data: any) {
  if (
    (data.application === "AGI" || data.application === "MixMonitor") &&
    data.appdata?.match(".WAV")
  ) {
...
export function resolveBranchOverride(
  branchState: any,
  channel: string,
  state: any,
  exten: string,
...
export function applyChannelStateUpdate(
  state: any,
  destchannel: string,
  data: any,
  status: string,
...
export function buildMasterHangupOverride(
  state: any,
  data: any,
  wasAnswered = false,
) {
...
export function updateMasterWebhookIfMissing(state: any, webhook_select: any) {
  if (!state.webhookurl && webhook_select) {
    ...
    state.connector_server = webhook_select.connector_server || "";
  }
}
...
export function storeQueueCaller(
  store: StoreService,
  data: any,
  pbxId: string,
) {
...
export function buildQueueSummaryWebhook(
  arrWebhook: any,
  data: any,
): Array<{ url: string; params: any }> {
...
export function buildDeviceStateWebhook(
  arrWebhook: any,
  data: any,
): Array<{ url: string; params: any }> {
...
export function isValidCdrEvent(data: any): boolean {
  if (!data.destinationchannel || data.destinationchannel.startsWith("AppDial"))
    return false;
...
export function handleNoAnsweredCalls(
  state: any,
  branchState: any,
  channel: string,
  branchKey: string,
  ...
  const isAnsweredBranch =
    state.status === "answered" && channel === state.destchannel;
